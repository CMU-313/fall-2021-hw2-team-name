...
api_urls = [
    ...
    url(
        regex=r'^documents/(?P<pk>[0-9]+)/resolved_web_links/(?P<resolved_web_link_id>[0-9]+)/$',
        name='resolved_web_link-detail',
        view=APIResolvedWebLinkView.as_view()
    ),
    url(
        regex=r'^documents/(?P<pk>[0-9]+)/resolved_web_links/(?P<resolved_web_link_id>[0-9]+)/navigate/$',
